...
setwd('/Users/liupeiwen/BC/New_analysis/1.Toomanycells_result/Bcell/')

cluster <- c('t_Bfoc-MKI67', 't_Bfoc-NEIL1', 't_Bmem-CD27', 't_Bmem-MKI67',
             't_Bn-TCL1A', 't_pB-IGHG1')
for(j in cluster){
  ...
  adata$label <- ifelse(adata$SD_Prop >= adata[1,'SD_Prop']+0.25 & adata$p_Val < 1e-3,"NR_enrich",
                        ifelse(adata$SD_Prop <= adata[1,'SD_Prop']-0.25 & adata$p_Val < 1e-3,"R_enrich","None_enrich"))
